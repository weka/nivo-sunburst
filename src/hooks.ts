...
export const useSunburst = <RawDatum>({
    ...
    const nodes: ComputedDatum<RawDatum>[] = useMemo(() => {
        ...
        // As nodes are already sorted by depth, we can get the max depth from the last node
        const maxDepth = sortedNodes.length > 0 ? sortedNodes[sortedNodes.length - 1].depth : 0
